...
class ConsultationRequestServices {
  ...
  Future<List> getAllConsultationRequest()async{
    ...
    var request = http.MultipartRequest('GET', Uri.parse('localhost:8000/api/consultation_requests'));
...
Future<String> update_consultationRequest(String subject,int id)async{
  var headers = {
    'Accept': 'application/json',
    'Content-Type': 'application/x-www-form-urlencoded',
    'Authorization': 'Bearer $myToken'
  };
  var request = http.Request('PUT', Uri.parse('${myUrl}consultation_requests/$id'));
  request.bodyFields = {
    'subject': subject
  };
  request.headers.addAll(headers);
...
Future<String>updateConsultationRequestStatus(int id,String status)async{
  ...
  var request = http.Request('PUT', Uri.parse('${myUrl}consultation_requests/status/$id'));
  request.bodyFields = {
    'status': status
  };
  request.headers.addAll(headers);
...
Future<String>deleteConsultationRequest (int id)async{
  ...
  var request = http.Request('DELETE', Uri.parse('${myUrl}consultation_requests/$id'));
  request.bodyFields = {};
  request.headers.addAll(headers);
  var streamedResponse = await request.send();
  var response = await http.Response.fromStream(streamedResponse);
