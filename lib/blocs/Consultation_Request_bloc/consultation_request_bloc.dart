...
import 'package:graduation/data/repositories/consultation_request_repository.dart';
...
class ConsultationRequestBloc extends Bloc<ConsultationRequestEvent, ConsultationRequestState> {
  ConsultationRequestBloc() : super(ConsultationRequestInitial()) {
    on<ConsultationRequestEvent>((event, emit) async {
      ...
      if (event is UpdateConsultationRequestEvent) {
        emit(
          ConsultationRequestLoading(),
        );
        try {
          String value = await ConsultationRequestServices()
              .update_consultationRequest(event.subject, event.id);
          emit(
            ConsultationRequestSuccess(
              successmsg: value,
            ),
      ...
      if (event is GetConsultationRequestStatusEvent) {
        emit(
          ConsultationRequestLoading(),
        );
        try {
      ...
      if (event is GetAllConsultationRequestStatusEvent) {
        emit(
          ConsultationRequestLoading(),
        );
        try {
         List< ConsultationRequestModel> value = await ConsultationRequestRepository()
              .ge_ALLConsultationRequest();
          emit(
           ConsultationRequestListLoadedSuccessFully(consultationRequest: value)
          );
        } catch (e) {
          emit(
            ConsultationRequestFail(
      ...
      if (event is DeleteConsultationRequestStatusEvent) {
        emit(
          ConsultationRequestLoading(),
        );
        try {
          ...
          emit(
              ConsultationRequestSuccess(successmsg: value)
          );
        } catch (e) {
          emit(
            ConsultationRequestFail(
