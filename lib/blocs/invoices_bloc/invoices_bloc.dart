      ...
      if (event is AddInvoiceEvent) {
        ...
        try {
          String result = await InvoiceServices()
              .addInvoice(event.issueId,event.userId,event.status, event.amount,);
        ...
        emit(InvoiceLoading());
        try {
          InvoiceModel invoice = await InvoiceServices()
              .getInvoiceById(event.invoiceId);
