  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
            create: (_) =>
                IssuesBloc()..add(IssueShowbyId(id: invoiceModel.issueId))),
        BlocProvider(
            create: (_) =>
                UserBloc()..add(GetUserById(userId: invoiceModel.userId))),
      ],
      child: Scaffold(
          ...
          child: BlocBuilder<UserBloc, UserState>(
            builder: (context, userState) {
              if (userState is UserLoadedSuccessfully) {
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 10),
                                _buildInfoRow(
                                    Icons.person, 'Name', user.name, 22, 16),
                                _buildInfoRow(
                                    Icons.email, 'Email', user.email, 22, 16),
                                _buildInfoRow(
                                    Icons.badge, 'Role', user.roleName, 22, 16),
                                ...
                                _buildInfoRow(
                                    Icons.title, 'Title', issue.title, 22, 16),
                                _buildInfoRow(Icons.confirmation_number,
                                    'Issue Number', issue.issueNumber, 22, 16),
                                //   _buildInfoRow(Icons.category, 'Category', issue.category, 22, 16),
                                _buildInfoRow(Icons.person, 'Opponent',
                                    issue.opponentName, 22, 16),
                                _buildInfoRow(Icons.account_balance, 'Court',
                                    issue.courtName, 22, 16),
                                _buildInfoRow(Icons.payment, 'Payments',
                                    issue.numberOfPayments.toString(), 22, 16),
                                _buildInfoRow(Icons.attach_money, 'Total Cost',
                                    issue.totalCost, 22, 16),
                                _buildInfoRow(Icons.money_off, 'Amount Paid',
                                    issue.amountPaid, 22, 16),
                ...
                return Center(
                    child: Text('Failed to load user: ${userState.errmsg}'));
              } else {
                return const Center(child: CircularProgressIndicator());
              }
            },
