...
class LatestNewsList extends StatefulWidget {
  const LatestNewsList({super.key, required this.bloc});
...
class _LatestNewsListState extends State<LatestNewsList> {
  ...
  void initState() {
    super.initState();
    widget.bloc.add(LegalNewsLatestEvent()); // أو أي حدث يناسب تحميل الأخبار
  }
        ...
        builder: (context, state) {
          if (state is LegalNewsListLoaded) {
            ...
            if ( legalNewsList.isEmpty) {
              return const Center(child: Text('There are no news'));
            }
            return Expanded(
