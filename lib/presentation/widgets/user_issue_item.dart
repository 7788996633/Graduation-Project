...
class _UserIssueItemState extends State<UserIssueItem> with TickerProviderStateMixin {
  ...
  // هنا نحدد إذا كانت القضية مؤرشفة أم لا، ويمكن تحديثها حسب الحاجة
                  ...
                  children: [
                    ...
                    IconButton(
                      onPressed: () {
                        ...
                        if (isArchived) {
                          archiveBloc.add(AddUnArchiveIssue(issueId: widget.issuesModel.id));
                        } else {
                          archiveBloc.add(AddArchiveEvent(issueId: widget.issuesModel.id));
                        }
                        setState(() {
                      ...
                      icon: Icon(
                        ...
                        color: isArchived ? Colors.grey[700] : AppColors.darkBlue,
                      ),
                      tooltip: isArchived ? 'إلغاء الأرشفة' : 'أرشفة',
                    ),
                  ],
                ),
                  ...
                  SizedBox(
                    ...
                    child: Stack(
                      children: [
                        ...
                        Positioned(
                          width: (currentPaidAmount * animation.value * 400).clamp(0.0, 400.0),
                          child: Container(
                            decoration: BoxDecoration(
                              borderRadius: BorderRadius.circular(25),
                          ...
                          if (priorityToString(issuePriority).toLowerCase() !=
                            ...
                            widget.issuesBloc.add(
                              UpdateIssuePriorityEvent(
                                ...
                                priority: priorityToString(issuePriority).toLowerCase(),
                              ),
                            );
                          }
                          if (statusToString(issueStatus).toLowerCase() !=
                            ...
                            widget.issuesBloc.add(
                              UpdateIssueStatusEvent(
                                issueId: widget.issuesModel.id,
                                status: statusToString(issueStatus).toLowerCase(),
                              ),
                            );
                          }
                        }
